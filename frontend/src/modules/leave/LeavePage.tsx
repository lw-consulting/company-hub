...
const STATUS_CONFIG: Record<string, { label: string; class: string; icon: any }> = {
  ...
  cancelled: { label: 'Storniert', class: 'badge bg-slate-100 text-slate-500', icon: XCircle },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-800">Meine Anträge</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-secondary border-b border-border">
                <th className="text-left text-xs font-semibold text-slate-500 uppercase px-6 py-3">Typ</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase px-6 py-3">Zeitraum</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase px-6 py-3">Tage</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase px-6 py-3">Status</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase px-6 py-3">Eingereicht</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-light">
              {!requests?.length ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-400">Keine Anträge vorhanden</td></tr>
              ) : (
                requests.map((req) => {
                    ...
                    <tr key={req.id} className="hover:bg-surface-secondary/50">
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-slate-700">{req.leaveTypeName}</span>
                        </div>
                      </td>
                      <td className="px-6 py-3 text-sm text-slate-600">
                        {formatDate(req.startDate)} - {formatDate(req.endDate)}
                      </td>
                      <td className="px-6 py-3 text-sm font-medium text-slate-700">{req.businessDays}</td>
                      <td className="px-6 py-3">
      ...
      {isManager && pendingRequests && pendingRequests.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-slate-800">Zu genehmigen</h2>
  ...
  return (
    <div className="card p-4 flex items-center justify-between">
      <div>
        ...
        {request.reason && <div className="text-xs text-slate-400 mt-1">{request.reason}</div>}
      </div>
      <div className="flex items-center gap-2">
        <button
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card p-6 w-full max-w-lg">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-slate-800">Neuer Antrag</h3>
